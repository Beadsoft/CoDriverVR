...
namespace dx {
    ...
    void render_passenger_target(void* p, RenderTarget target)
    {
        g::vr_render_target = target;
        if (g::vr->prepare_vr_rendering(g::d3d_dev, target, true)) {
            g::hooks::render.call(p);
            g::vr->finish_vr_rendering(g::d3d_dev, target);
        } else {
    ...
    void render_passenger_eyes(void* p)
    {
        ...
        render_passenger_target(p, PassengerLeft);
        if (g::cfg.passenger_vr.render_mode == "stereo") {
            render_passenger_target(p, PassengerRight);
        }
    ...
    HRESULT __stdcall Present(IDirect3DDevice9* This, const RECT* pSourceRect, const RECT* pDestRect, HWND hDestWindowOverride, const RGNDATA* pDirtyRegion)
    {
        ...
        if (g::vr && !g::vr_error) {
            ...
            if (g::cfg.passenger_vr.enabled && rbr::is_rendering_3d() && game_mode != GameMode::MainMenu) {
                render_passenger_companion_window(g::d3d_dev, g::vr);
        ...
        if (!shader && Vector4fCount == 4) {
            ...
            if (g::vr_render_target) {
                ...
                if (StartRegister == 0) {
                    ...
                    const auto pose_target = (target == PassengerLeft || target == PassengerRight) ? target : LeftEye;
