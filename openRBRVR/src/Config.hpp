...
struct Config {
    ...
    static Config from_toml(const std::filesystem::path& path)
    {
        ...
        cfg.swap_eyes = parsed["swapEyes"].value_or(false);
        ...
        if (passenger_vr_node.is_table()) {
            ...
            cfg.passenger_vr.render_mode = passenger_vr_node["renderMode"].value_or("stereo");
