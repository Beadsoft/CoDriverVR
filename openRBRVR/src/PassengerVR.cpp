...
namespace passenger_vr {
    namespace {
        ...
        void handle_control_packet(const std::string& payload)
        {
            std::stringstream ss(payload);
            std::string type;
            ss >> type;
            if (type == "handPose") {
                ...
                float qw = 1.0f;
                int pinch = 0;
                if (ss >> side >> valid >> x >> y >> z >> qx >> qy >> qz >> qw >> pinch) {
                    roadbook_vr::update_passenger_hand_state(side == "right" ? "right" : "left", valid != 0, { x, y, z }, glm::quat(qw, qx, qy, qz), pinch != 0);
                }
            } else if (type == "roadbook") {
                std::string command;
                if (ss >> command) {
                    roadbook_vr::queue_passenger_command(command);
                }
            }
        }
    }
    ...
    void poll_pose_receiver()
    {
        ...
        for (;;) {
            ...
            buffer[received] = '\0';
            const auto payload = std::string(buffer.data(), static_cast<size_t>(received));
            if (payload.rfind("handPose ", 0) == 0 || payload.rfind("roadbook ", 0) == 0) {
                handle_control_packet(payload);
                continue;
            }

            const auto values = parse_floats(payload);
            if (values.size() >= 3) {
