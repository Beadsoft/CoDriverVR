...
namespace g {
    ...
    static IDirect3DVertexBuffer9* passenger_companion_vertex_buf_left;
    static IDirect3DVertexBuffer9* passenger_companion_vertex_buf_right;
...
void VRInterface::update_passenger_pose()
{
    if (!g::cfg.passenger_vr.enabled) {
        return;
    }

    passenger_vr::start_pose_receiver(g::cfg.passenger_vr);
    passenger_vr::poll_pose_receiver();
    projection[PassengerLeft] = projection[LeftEye];
    projection[PassengerRight] = projection[RightEye];
    eye_pos[PassengerLeft] = eye_pos[LeftEye];
    eye_pos[PassengerRight] = eye_pos[RightEye];
    hmd_pose[PassengerLeft] = passenger_vr::view_matrix(g::cfg.passenger_vr);
    hmd_pose[PassengerRight] = hmd_pose[PassengerLeft];
}
...
static bool create_passenger_companion_window_buffers(IDirect3DDevice9* dev)
{
    // clang-format off
    Vertex left[] = {
        { -1,  1, 1, 0, 0, },
    ...
    if (g::passenger_companion_vertex_buf_left) {
        g::passenger_companion_vertex_buf_left->Release();
    }
    if (g::passenger_companion_vertex_buf_right) {
        g::passenger_companion_vertex_buf_right->Release();
    }
    if (!create_vertex_buffer(dev, left, 4, &g::passenger_companion_vertex_buf_left)) {
        dbg("Could not create left passenger companion vertex buffer");
        return false;
    }
    if (!create_vertex_buffer(dev, right, 4, &g::passenger_companion_vertex_buf_right)) {
        dbg("Could not create right passenger companion vertex buffer");
        return false;
    }
    return true;
}
...
void VRInterface::init_surfaces(IDirect3DDevice9* dev, RenderContext& ctx, uint32_t res_x_2d, uint32_t res_y_2d)
{
    ...
    if (g::cfg.passenger_vr.enabled) {
        if (ctx.width[PassengerLeft] == 0 || ctx.height[PassengerLeft] == 0) {
            ctx.width[PassengerLeft] = ctx.width[LeftEye];
            ctx.height[PassengerLeft] = ctx.height[LeftEye];
        }
        if (ctx.width[PassengerRight] == 0 || ctx.height[PassengerRight] == 0) {
            ctx.width[PassengerRight] = ctx.width[RightEye];
            ctx.height[PassengerRight] = ctx.height[RightEye];
        }
        create_vr_render_target(PassengerLeft);
        create_vr_render_target(PassengerRight);
    }
    ...
    if (!quads_created) {
        ...
        if (!create_passenger_companion_window_buffers(dev))
            throw std::runtime_error("Could not create passenger stereo desktop window buffers");
...
void render_textured_quad(IDirect3DDevice9* dev, VRInterface* vr, IDirect3DTexture9* texture, RenderTarget render_target_3d, IDirect3DVertexBuffer9* vbuf, const M4& model)
{
    const auto left = render_target_3d;
    const auto& projectionl = vr->get_projection(left);
    const auto& eyeposl = vr->get_eye_pos(left);
    ...
    D3DMATRIX mvpr = { 0 };
    if (dx::multiview_rendering_enabled() && (left == LeftEye || left == FocusLeft)) {
        const auto right = render_target_counterpart(left);
        const auto& projectionr = vr->get_projection(right);
        const auto& eyeposr = vr->get_eye_pos(right);
        const auto& poser = vr->get_pose(right);
        mvpr = d3d_from_m4(projectionr * eyeposr * poser * g::flip_z_matrix * model);
    }

    const D3DMATRIX mvpl = d3d_from_m4(projectionl * eyeposl * posel * g::flip_z_matrix * model);
    render_texture(dev, &mvpl, &mvpr, &g::identity_matrix, &g::identity_matrix, texture, vbuf);
}
...
void render_passenger_companion_window(IDirect3DDevice9* dev, VRInterface* vr)
{
    if (g::cfg.passenger_vr.render_mode == "stereo") {
        render_texture(dev, &g::identity_matrix, &g::identity_matrix, &g::identity_matrix, &g::identity_matrix, vr->get_texture(PassengerLeft), g::passenger_companion_vertex_buf_left);
        render_texture(dev, &g::identity_matrix, &g::identity_matrix, &g::identity_matrix, &g::identity_matrix, vr->get_texture(PassengerRight), g::passenger_companion_vertex_buf_right);
        return;
    }

    render_companion_window_from_render_target(dev, vr, PassengerLeft);
}
