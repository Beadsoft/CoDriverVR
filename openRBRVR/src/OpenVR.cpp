...
void OpenVR::init(IDirect3DDevice9* dev, IDirect3DVR9** vrdev, uint32_t companionWindowWidth, uint32_t companionWindowHeight)
{
    ...
    try {
        for (const auto& gfx : g::cfg.gfx) {
            ...
            RenderContext ctx = {
                .width = { wss, wss, wss, wss, wss, wss },
                .height = { hss, hss, hss, hss, hss, hss },
...
void OpenVR::submit_frames_to_hmd(IDirect3DDevice9* dev)
{
    ...
    const auto submit_left = g::cfg.swap_eyes ? RightEye : LeftEye;
    const auto submit_right = g::cfg.swap_eyes ? LeftEye : RightEye;
    if (auto e = compositor->Submit(static_cast<vr::EVREye>(LeftEye), &openvr_texture[submit_left]); e != vr::VRCompositorError_None) [[unlikely]] {
        dbg(std::format("Compositor error: {}", vr_compositor_error_str(e)));
    }
    if (auto e = compositor->Submit(static_cast<vr::EVREye>(RightEye), &openvr_texture[submit_right]); e != vr::VRCompositorError_None) [[unlikely]] {
        dbg(std::format("Compositor error: {}", vr_compositor_error_str(e)));
    }
...
bool OpenVR::update_vr_poses()
{
    ...
    roadbook_vr::update_openvr_controller_state(hmd, poses);
