...
struct RenderContext {
    // One for each possible runtime/passenger view.
    uint32_t width[VrViewCount] = { 0 };
    uint32_t height[VrViewCount] = { 0 };
    HANDLE dx_shared_handle[VrViewCount] = { 0 };

    // One for each render target
    IDirect3DTexture9* dx_texture[RenderTargetCount] = { 0 };
    IDirect3DSurface9* dx_surface[RenderTargetCount] = { 0 };
    IDirect3DSurface9* dx_depth_stencil_surface[RenderTargetCount] = { 0 };
...
protected:
    ...
    M4 hmd_pose[VrViewCount];
    M4 eye_pos[VrViewCount];
    M4 projection[VrViewCount];
