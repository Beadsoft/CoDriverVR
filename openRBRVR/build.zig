...
pub fn build(b: *std.Build) void {
    ...
    codriver.linkLibC();
    codriver.addCSourceFiles(.{ .files = &.{
        "src/CoDriverVRAPI.cpp",
        "src/CoDriverVRPlugin.cpp",
        "src/DriverRoomControl.cpp",
    }, .flags = &.{
        "-Wno-ignored-attributes",
        "--std=c++23",
    } });
    codriver.linkSystemLibrary("wininet");
    b.installArtifact(codriver);

    // For compile_commands.json
    var targets: std.ArrayListUnmanaged(*std.Build.Step.Compile) = .empty;
    targets.append(b.allocator, dll) catch @panic("OOM");
    targets.append(b.allocator, codriver) catch @panic("OOM");
