...
- Gated passenger stereo render targets.
- Passenger view matrix using a co-driver seat offset plus remote yaw/pitch/roll.
- UDP pose input on `posePort`.
- Desktop companion-window side-by-side output from the passenger render targets while passenger mode is enabled.
...
- The Quest Browser client expects the passenger companion window as side-by-side stereo; native Unity packaging is scaffolded but not yet wired to a scene.
