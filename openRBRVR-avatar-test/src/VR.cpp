...
void VRInterface::update_passenger_pose()
{
    ...
    projection[PassengerMono] = projection[LeftEye];
    eye_pos[PassengerMono] = glm::identity<M4>();
    hmd_pose[PassengerMono] = passenger_vr::view_matrix(g::cfg.passenger_vr);
}
...
void VRInterface::init_surfaces(IDirect3DDevice9* dev, RenderContext& ctx, uint32_t res_x_2d, uint32_t res_y_2d)
{
    ...
    if (g::cfg.passenger_vr.enabled) {
        if (ctx.width[PassengerMono] == 0 || ctx.height[PassengerMono] == 0) {
            ctx.width[PassengerMono] = ctx.width[LeftEye];
            ctx.height[PassengerMono] = ctx.height[LeftEye];
        }
        create_vr_render_target(PassengerMono);
    }
