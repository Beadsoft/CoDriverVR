...
namespace dx {
    ...
    namespace fixedfunction {
        static D3DMATRIX current_projection_matrix[VrViewCount];
        static D3DMATRIX current_view_matrix[VrViewCount];
        static D3DMATRIX current_world_matrix;
        static D3DMATRIX current_projview_matrix[VrViewCount];
        static D3DMATRIX current_viewproj_matrix[VrViewCount];
        static D3DMATRIX current_worldview_matrix[VrViewCount];
        static D3DMATRIX current_worldviewproj_matrix[VrViewCount];
    }
    ...
    void render_passenger_eye(void* p)
    {
        if (!g::cfg.passenger_vr.enabled) {
            return;
        }

        const auto multiview_was_enabled = g::cfg.multiview;
        g::cfg.multiview = false;
        g::vr_render_target = PassengerMono;
        if (g::vr->prepare_vr_rendering(g::d3d_dev, PassengerMono, true)) {
            g::hooks::render.call(p);
            g::vr->finish_vr_rendering(g::d3d_dev, PassengerMono);
        } else {
            dbg("Failed to set passenger render target");
            g::d3d_dev->SetRenderTarget(0, g::original_render_target);
            g::d3d_dev->SetDepthStencilSurface(g::original_depth_stencil_target);
        }
        g::vr_render_target = std::nullopt;
        g::cfg.multiview = multiview_was_enabled;
    }
    ...
    static void draw_debug_info(uint64_t cpu_frametime_us)
    {
        ...
        if (g::cfg.debug_mode == 0) {
            ...
            if (g::cfg.passenger_vr.enabled) {
                const auto pose = passenger_vr::latest_pose_degrees();
                const auto pose_age = passenger_vr::latest_pose_age_ms();
                const auto pose_age_text = pose_age < 0 ? "never" : std::format("{}ms", pose_age);
                g::game->WriteText(0, 18 * ++i,
                    std::format("PassengerVR: pose yaw {:.1f} pitch {:.1f} roll {:.1f}, age {}", pose.x, pose.y, pose.z, pose_age_text).c_str());
            }
            if (g::cfg.roadbook_vr.enabled) {
                g::game->WriteText(0, 18 * ++i, roadbook_vr::debug_status().c_str());
            }
    ...
    HRESULT __stdcall Present(IDirect3DDevice9* This, const RECT* pSourceRect, const RECT* pDestRect, HWND hDestWindowOverride, const RGNDATA* pDirtyRegion)
    {
        if (g::vr && !g::vr_error) [[likely]] {
            ...
            roadbook_vr::render(g::d3d_dev, g::vr);
        }
        ...
        if (g::vr && !g::vr_error) {
            ...
            if (g::cfg.passenger_vr.enabled && rbr::is_rendering_3d() && game_mode != GameMode::MainMenu) {
                render_companion_window_from_render_target(g::d3d_dev, g::vr, PassengerMono);
        ...
        if (!shader && Vector4fCount == 4) {
            ...
            if (g::vr_render_target) {
                ...
                if (StartRegister == 0) {
                    ...
                    const auto pose_target = target == PassengerMono ? PassengerMono : LeftEye;
                    glm::decompose(g::vr->get_pose(pose_target), scale, orientation, translation, skew, perspective);
    ...
    {
        asset_probe::log_draw_primitive(g::d3d_dev, PrimitiveType, StartVertex, PrimitiveCount);
    ...
    {
        asset_probe::log_draw_indexed_primitive(g::d3d_dev, PrimitiveType, BaseVertexIndex, MinVertexIndex, NumVertices, startIndex, primCount);
