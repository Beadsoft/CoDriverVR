...
void OpenXR::init(IDirect3DDevice9* dev, IDirect3DVR9** vrdev, uint32_t companion_window_width, uint32_t companion_window_height, std::optional<XrPosef> old_view_pose)
{
    ...
    eye_pos[PassengerMono] = glm::identity<glm::mat4x4>();
...
{
    if (input_state.action_set == XR_NULL_HANDLE || input_state.pose_action == XR_NULL_HANDLE) {
        return;
    }
    ...
    for (;;) {
        ...
        if (eventAvailable == XR_EVENT_UNAVAILABLE) {
            break;
        }
    ...
    std::array<M4, Side::COUNT> grip_pose { glm::identity<M4>(), glm::identity<M4>() };
    std::array<bool, Side::COUNT> grip_valid { false, false };
    for (const auto hand : { Side::LEFT, Side::RIGHT }) {
        ...
        if (pose_state.isActive) {
            ...
            const bool valid = res == XR_SUCCESS
                && (space_location.locationFlags & XR_SPACE_LOCATION_POSITION_VALID_BIT) != 0
                && (space_location.locationFlags & XR_SPACE_LOCATION_ORIENTATION_VALID_BIT) != 0;
            if (valid) {
                grip_pose[hand] = xr_pose_to_m4(space_location.pose);
                grip_valid[hand] = true;
            }
        }
    }

    avatar_hands::update_from_openxr_grip_pose(grip_pose[Side::LEFT], grip_valid[Side::LEFT], grip_pose[Side::RIGHT], grip_valid[Side::RIGHT]);
}
...
{
    passenger_vr::stop_pose_receiver();
    ...
    for (const auto& v : render_contexts) {
        ...
        for (int i = 0; i < RenderTargetCount; ++i) {
            ...
            if (i < VrViewCount && ctx.dx_shared_handle[i] != nullptr && ctx.dx_shared_handle[i] != INVALID_HANDLE_VALUE) {
